...
public class Main {
     ...
     * 右边都是大于pivot 的元素。使用递归不断对左、右两边元素采用上述逻辑进行比较，左、右，两边元素无法递
     * 归时，结束。那结束递归的条件是什么？
     *   我们来解读一下上述原理。可以看出，快速排序包含两步操作：选出pivot 元素并将它放到合适位置；对左、右两边
     * 元素进行递归调用检查。快速排序与归并排序类似，采用分治思想解决问题。我们先写出它的递推公式：
     * quickSort(s...e)=quickSort(s, pivotIndex) + quick(pivotIndex + 1, e)。可以看出，对于左、右
     * 两边递归调用比较简单。难点在于选出pivot 元素并将它放到合适的位置。一旦涉及将元素放到数组中合适的位置，
     * 就需要移动数组内的数据，以实现数组内数据的连续性。整个操作下来时间复杂度为O(n^2)，通过使用替换方式，将
     * 时间复杂度降至O(1)。
     *   对于选取pivot 元素，默认选数组中最后一个元素。首先将数组内元素进行划分，分为已处理、未处理。区域
     * 的划分，并不使用格外的数组进行存储，那样快速排序的空间复杂度会上升。这里的划分只是逻辑上的划分，指定下
     * 标内的元素为已处理，下标外的元素为未处理。划分处理方式有点像选择排序，第一个元素下标默认已处理，第二个
     * 元素以后为未处理。
     *   使用一个变量i，将s~e 之间的分成两部分，数组[s-i]为已处理部分，数组[i+1,e] 为未处理部分。我们在
     * s ~ e之间选一个元素数组[j]，与pivot 比较。如果比pivot 小，则加入到已处理区间，i++，表示已处理。
     * 如果比pivot大，移动变量j移动。
     *
     * 伪代码：
